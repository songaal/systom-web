<template>
  <div>
    <a href="javascript:void(0);" @click="showModal"><u>플랜재사용</u></a>

    <b-modal id="planReUseModal" no-fade>
      <template slot="modal-header">
        <h5>플랜재사용</h5>
      </template>
      <b-alert show variant="info" class="ws-pre-line">플랜을 다시 사용하시겠습니까?</b-alert>
      <template slot="modal-footer">
        <b-button @click="(e) => this.$root.$emit('bv::hide::modal', 'planReUseModal')">취소</b-button>
        <b-button variant="info"
                  @click="el => reUse(el)">플랜재사용</b-button>
      </template>
    </b-modal>
...
export default {
  ...
  methods: {
    ...
    reUse (el) {
      ...
      let url = `${config.serverHost}/${config.serverVer}/paidPlan?action=reUse`
